...
class Ion_auth_model extends DT_CI_Model
{
	/**
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
        ...
        $samajId = $additional_data['samaj_id'];
        unset($additional_data['samaj_id']);
		...
		# user add multiple company

        if (isset($samajId) && is_array($samajId) && !empty($samajId) && count(array_filter($samajId)) > 0) {
            foreach ($samajId as $samajIdKey => $samajIdValue) {
                $userSamajArray[] = array(
                    'users_samaj_id'  => '',
                    'users_id'        => $id,
                    'samaj_id'        => $samajIdValue
                );
            }
            $this->db->insert_batch('tbl_users_samaj', $userSamajArray);
        }else if(isset($samajId) && $samajId != ''){
			$userSamajArray[] = array(
				'users_samaj_id'  => '',
				'users_id'        => $id,
				'samaj_id'        => $samajId
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,GROUP_CONCAT(samaj_id SEPARATOR ",") as samaj_id')
		                  ->where($this->identity_column, $identity)
		                  ->join('tbl_users_samaj as us', "us.users_id = ".$this->tables['users'].".id ",'left')
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->update_last_login($user->id, $loginToken);
...
//            $this->db->delete('tbl_users_company');
            $this->db->delete('tbl_users_samaj');

            # user add multiple company
            if (isset($samajArray) && !empty($samajArray) && count(array_filter($samajArray)) > 0) {
                foreach ($samajArray as $key => $samajId) {
                    $userSamajArray[] = array(
                        'users_samaj_id'  => '',
                        'users_id'        => $user->id,
                        'samaj_id'        => $samajId
...
//        }
	...
	**/
	 ...
	 * @author Ben Edmunds
	 **/
	public function update_last_login($id, $loginToken)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time(), 'login_token' => $loginToken), array('id' => $id));
    ...
    public function set_session_email_id($user)
    {
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
            ...
            $this->set_session_email_id($user);
    ...
    public function checkDuplicateEmail($id = 0, $email)
    {
        $this->db->select("*");
...
//		echo "A{$group_id}A";
	...
	**/
	...
	public function otpLatestUpdate($emailId,$otp){
        $this->db->set('is_latest',0);
        $this->db->where('email',$emailId);
        $this->db->where('otp',$otp);
        $return =  $this->db->update('tbl_password_reset');
        return $return;
    }

    public function getUsersDD($filterParameter = '',$page = 1, $userId = array(), $start = 0, $limit = 10)
    {
        if($page != 1){
        ...
        $this->db->start_cache();
        $this->db->select("id as id,concat(first_name,' ', last_name) as text");
        $this->db->from('tbl_users');
        if (!empty($userId)) {
            $this->db->where_in('id', $userId);
        }
        if ($filterParameter != '') {
            $this->db->like('username', $filterParameter, 'both');
        }
        $this->db->stop_cache();
    ...
    public function userSamaj($userId = ''){
        $this->db->select('users_samaj.samaj_id,samaj_desc.samaj_name');
        $this->db->from("tbl_users_samaj as users_samaj");
        $this->db->join('tbl_samaj_description as samaj_desc','samaj_desc.samaj_id = users_samaj.samaj_id','left');
        $this->db->join('tbl_language_master as language','language.language_id = samaj_desc.language_id','left');
        $this->db->where('language.is_default', 1);
        $this->db->where('users_samaj.users_id', $userId);
        $result = $this->db->get()->result_array();
        return $result;
    }

    public function userCompanyIdName($companyId = array())
    {

        $this->db->select("samaj_id,samaj_name");
        $this->db->from("tbl_company");

        if(!empty($companyId)) {
            $this->db->where_in('samaj_id', $companyId);
        }else{
            $this->db->where('samaj_id', 0);
        }
...
//    public function userCompanySee($userId = '')
//    {
//        $this->db->select("groups.see_all_data");
//        $this->db->from("tbl_users_groups as users_groups");
//        $this->db->join("tbl_groups as groups","groups.id = users_groups.group_id ","left");
//        $this->db->where('users_groups.user_id', $userId);
//        $seeAllData = $this->db->get()->row_array();
//
//
//        if($seeAllData['see_all_data'] == 1) {
//        $this->db->select("GROUP_CONCAT(distinct users_company.company_id SEPARATOR ',') as company_id");
//        $this->db->from("tbl_users_company as users_company");
//        $this->db->where('users_company.users_id', $userId);
//        }else {
//            $this->db->select("GROUP_CONCAT(distinct users_company.company_id SEPARATOR ',') as company_id");
//            $this->db->from("tbl_users_company as users_company");
//            $this->db->where('users_company.users_id', $userId);
