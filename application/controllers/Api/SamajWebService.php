...
//		$memberData['city']             = $this->Mdl_member->getMemberExtraData($memberExtraData, 6);
	...
	/**@api {post} Api/SamajWebService/getStateListing State-List
	 * @apiName State List
	 * @apiHeader {string} API-KEY=123456 API-KEY For User
	 * @apiGroup State
	 * @apiVersion 0.1.0
	 ...
	 * @apiUse SuccessDetails
	 *
	 * @apiSuccessExample {json} getStateListing
	 * {
	 *   {
	 *           "status": true,
	 *           "responseCode": 200,
	 ...
	 * @apiUse UnknownMethod
	 *
	 * @apiSampleRequest Api/SamajWebService/getStateListing
	 */
	public function getStateListing_post()
	{
		...
		$this->form_validation->set_rules('start', 'Start', 'required|integer');
		$this->form_validation->set_message('required', '%s is required');
		$this->form_validation->set_message('integer', '%s Please Enter Digit Only');
		if ($this->form_validation->run() === FALSE) {
			$strip_message = strip_tags(validation_errors(""));
			$this->response(array(
				"status" => FALSE,
		...
		$stateData = $this->Mdl_state->getStateListing($stateArray);
		if (!empty($stateData)) {
			$this->response(array(
				'status'             => TRUE,
				'responseCode' => self::HTTP_OK,
	...
	/**@api {post} Api/SamajWebService/getCityListing City-List
	 * @apiName City List
	 * @apiHeader {string} API-KEY=123456 API-KEY For User
	 * @apiGroup City
	 * @apiVersion 0.1.0
	 ...
	 * @apiUse SuccessDetails
	 *
	 * @apiSuccessExample {json} getCityListing
	 *   {
	 *             "status": true,
	 *               "responseCode": 200,
	 ...
	 * @apiUse UnknownMethod
	 *
	 * @apiSampleRequest Api/SamajWebService/getCityListing
	 */
	public function getCityListing_post()
	{
		...
		$cityData = $this->Mdl_city->getCityListing($cityArray);
		if (!empty($cityData)) {
			$this->response(array(
				'status'             => TRUE,
				'responseCode' => self::HTTP_OK,
