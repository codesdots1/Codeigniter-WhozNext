...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
       ...
       // $this->load->model(array('WebServiceModel'));
        ...
        //      DT_Code for auth Token

        $ignoreMethods = array('login','checkOtp','getLanguageList','getBusinessList','addEditBusiness','deleteBusiness',
        'addEditCategories','getCategoriesList','deleteCategories','getStaffList','addEditStaff','deleteStaff','getBusinessInfo',
        'addEditService','getServiceList','deleteService','addEditBooking','getBookingList','deleteBooking','addEditBookingPreferences','deviceTokenUpdate',
        'getGalleryList','addEditGallery','addEditCategoriesStatus','updateBookingStatus');
        $functionMethod = $this->uri->segment(3);

        if(isset($functionMethod) && !in_array($functionMethod,$ignoreMethods)) {

            $this->load->model(array('Mdl_employee'));
            $businessId = (isset($_POST['business_id']) && $_POST['business_id'] != '') ? $_POST['business_id'] : '';
            $authToken = (isset($_POST['auth_token']) && $_POST['auth_token'] != '') ? $_POST['auth_token'] : '';

            $tokenData = $this->Mdl_employee->getBusinessTokenData($businessId, $authToken);
    ...
    public function userAccess($userId, $authToken){

        $tokenData = $this->WebServiceModel->getUserTokenData($userId, $authToken);

        if (empty($tokenData)) {
            $this->response(array(
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => 'Invalid token',
        ...
        if (!empty($tokenData)) {
            if($tokenData[0]['auth_token_validity'] < date('Y-m-d H:i:s')){
                $this->response(array(
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => 'Auth token Expire',
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response([
                ...
                'responseCode'=>self::HTTP_FORBIDDEN
            ], self::HTTP_FORBIDDEN);
        ...
        if (!method_exists($this, $controller_method)) {
            $this->response([
                ...
                'responseCode'=>self::HTTP_METHOD_NOT_ALLOWED,
     ...
     */
    public function response($data = array(), $http_code = NULL, $continue = FALSE)
    {
        ...
        if (!$isProfilingEnabled) {
            ...
            if ($this->config->item('rest_enable_logging') === TRUE) {
                $this->_log_response_code($http_code,$output);
            }
    ...
    protected function _log_request($authorized = FALSE,$response="")
    {
        ...
        //echo "<pre>"; print_r($is_inserted); die();
     ...
     * @return bool TRUE log table updated; otherwise, FALSE
     */
    protected function _log_response_code($http_code,$output)
    {
        if($this->_insert_id == ''){
            return false;
        }
    ...
    public function checkToken($tokenData)
    {
        if (empty($tokenData)) {
            $this->response(array(
                $this->config->item('rest_status_field_name') => FALSE,
//                $this->config->item('rest_message_field_name') => 'Invalid token',
        ...
        if (!empty($tokenData)) {
            if($tokenData['auth_token_validity'] < date('Y-m-d H:i:s')){
                $this->response(array(
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => 'Auth token Expire',
                    'responseCode'=>self::HTTP_PAYMENT_REQUIRED
                ), REST_Controller::HTTP_PAYMENT_REQUIRED);
            }
        }
    }
